//! src/mesh/operations/xor.rs

use crate::mesh::{
    csg::{Polygon, PolygonShared, Vertex},
    Csg,
};
...
/// Computes the exclusive-OR of two meshes using the custom CSG implementation.
pub fn xor(mesh_a: &[Triangle], mesh_b: &[Triangle]) -> Result<Vec<Triangle>, &'static str> {
    ...
    if polygons_a.is_empty() {
        return Ok(polygons_to_triangles(&polygons_b));
    }
    if polygons_b.is_empty() {
        return Ok(polygons_to_triangles(&polygons_a));
    }
