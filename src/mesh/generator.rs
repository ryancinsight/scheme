...
use crate::mesh::primitives::{generate_cuboid, generate_cylinder, generate_sphere, generate_cone};
use stl_io::Triangle;
...
pub fn generate_mesh_from_system(system: &ChannelSystem3D) -> Vec<Triangle> {
    ...
    // Generate and add meshes for all cones.
    for cone in &system.cones {
        triangles.extend(generate_cone(cone));
    }
