...
pub fn create_dynamic_split_geometry(box_dims: (f64, f64), num_splits: u32) -> ChannelSystem {
...
pub fn create_trifurcation_geometry(box_dims: (f64, f64), num_splits: u32) -> ChannelSystem {
    const WALL_CLEARANCE: f64 = 4.0;
    let (length, width) = box_dims;
    let mut lines = Vec::new();
